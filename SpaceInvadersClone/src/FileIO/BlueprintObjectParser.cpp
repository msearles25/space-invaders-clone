...
void BlueprintObjectParser::parseNextObjectForBlueprint(
	std::ifstream& reader, GameObjectBluprint& bp)
{
	...
	while (std::getline(reader, lineFromFile))
	{
		if (lineFromFile.find(ObjectTags::COMPONENT) != std::string::npos)
		{
			value = extractStringBetweenTags(
				lineFromFile, ObjectTags::COMPONENT, ObjectTags::COMPONENT_END);

			bp.addComponentList(value);
		}
		...
		{
			value = extractStringBetweenTags(
				lineFromFile, ObjectTags::WIDTH, ObjectTags::WIDTH_END);
		...
		{
			value = extractStringBetweenTags(
				lineFromFile, ObjectTags::HEIGHT, ObjectTags::HEIGHT_END);
		...
		{
			value = extractStringBetweenTags(
				lineFromFile, ObjectTags::LOCATION_X, ObjectTags::LOCATION_X_END);
		...
		{
			value = extractStringBetweenTags(
				lineFromFile, ObjectTags::LOCATION_Y, ObjectTags::LOCATION_Y_END);
		...
		{
			value = extractStringBetweenTags(
				lineFromFile, ObjectTags::BITMAP_NAME, ObjectTags::BITMAP_NAME_END);
...
std::string BlueprintObjectParser::extractStringBetweenTags(
